...
function NodeTable({ nodes, labels, current, delays, busy, onSelect, onTest }) {
  ...
  return <table className="w-full"><thead><tr className="sticky top-0 border-b border-[#1e2230] bg-[#14161e] text-[11px] uppercase text-[#6b7280]"><th className="w-8 px-3 py-1.5" /><th className="px-3 py-1.5">Node</th><th className="px-3 py-1.5">Type</th><th className="px-3 py-1.5">Delay</th><th className="px-3 py-1.5" /></tr></thead><tbody>{nodes.map((name) => {
    ...
    return <tr key={name.name} className={`border-b border-[#1a1d27] ${selected ? "bg-blue-950/60 shadow-[inset_3px_0_0_#60a5fa]" : ""}`} aria-current={selected ? "true" : undefined}><td className="px-3 py-1.5 text-green-400">{selected ? "●" : pending ? "↻" : ""}</td><td className="px-3 py-1.5" title={name.name}><b className={selected ? "text-blue-200" : ""}>{labels[name.name] || name.name}</b><div className="max-w-56 overflow-hidden text-ellipsis whitespace-nowrap text-[10px] text-[#6b7280]">{name.name}</div></td><td className="px-3 py-1.5 text-[10px] text-[#6b7280]">{name.type || "-"}</td><td className={`px-3 py-1.5 ${pending ? "" : latencyClass(delay)}`}>{delay ? `${delay}ms` : pending ? "..." : "-"}</td><td className="px-3 py-1.5"><button className={iconButton} disabled={selected || pending} onClick={() => onSelect(name.name)}>use</button> <button className={iconButton} disabled={pending} onClick={() => onTest(name.name)}>test</button></td></tr>;
  })}</tbody></table>;
}
